...
{{-- Hero — text above, full composed scene below (fluid on all screens) --}}
<section class="hero-scene">
    <div class="container">
        <div class="hero-copy" data-reveal>
            <span class="welcome-pill">WELCOME TO</span>
            <h1 class="shine-line" style="margin-top:12px;">
                <span class="w1">Where Young Minds Learn,</span><br>
                <span class="w2">Create,</span> <span class="w3">Sing,</span> <span class="w4">Speak</span> <span class="w1">&</span> <span class="w5">Shine!</span>
            </h1>
            <p class="hero-sub">{{ $content['hero']['subheading'] }}</p>
            <div class="hero-cta flex gap-4" style="flex-wrap:wrap;justify-content:center;">
                <a href="{{ route('discovery') }}" class="btn-pink">✨ Book a Free Trial Class</a>
                <a href="{{ route('courses') }}" class="btn-purple">Explore Programs →</a>
            </div>
        </div>
        <div class="hero-art" data-reveal>
            <img src="{{ asset('images/hero-scene.png') }}" alt="Auntie Kash Kids — Auntie Kash with a rainbow, friendly dinosaur, sun and rocket">
        </div>
    </div>
</section>
...
{{-- Programs icon strip (matches mockup) --}}
<section style="background:#fff;padding:48px 0 40px;position:relative;z-index:3;">
    <div class="container">
        <h2 class="text-center" style="font-size:clamp(24px,3.5vw,34px);color:var(--navy);margin-bottom:32px;">Our Programs</h2>
        <div class="program-strip">
            @php
                $stripColors = ['#FF3E9E','#E67E22','#FFB400','#7ED321','#2AA7FF','#7B2FF7','#1ABC9C','#FF3E9E'];
            @endphp
            @foreach($featuredCourses->take(8) as $i => $course)
                <a href="{{ route('courses.show', $course->slug) }}" class="program-pill" data-reveal>
                    <span class="program-circle" style="background:{{ $stripColors[$i % count($stripColors)] }};">{{ $course->icon }}</span>
                    <span class="program-label">{{ $course->title }}</span>
                </a>
            @endforeach
            <a href="{{ route('courses') }}" class="program-pill" data-reveal>
                <span class="program-circle" style="background:#F4F1FB;border:2px dashed #C9B8F0;">▦</span>
                <span class="program-label">View All<br>Programs</span>
            </a>
        </div>
    </div>
</section>
<style>
.program-strip { display:grid; grid-template-columns: repeat(9, 1fr); gap:14px; }
.program-pill { display:flex; flex-direction:column; align-items:center; gap:10px; text-align:center; padding:12px 6px; border-radius:16px; transition:all 180ms; }
.program-pill:hover { background:#FAF7FF; transform:translateY(-3px); }
.program-circle { width:64px; height:64px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:28px; color:#fff; box-shadow:0 6px 16px rgba(0,0,0,0.12); }
.program-label { font-family:var(--font-fredoka); font-weight:600; font-size:13px; color:var(--navy); line-height:1.25; }
@media (max-width:1024px){ .program-strip{ grid-template-columns:repeat(5,1fr);} }
@media (max-width:600px){ .program-strip{ grid-template-columns:repeat(3,1fr);} .program-circle{width:54px;height:54px;font-size:24px;} }
</style>
...
<section class="section" style="background:var(--surface);">
    <div class="container">
        <div class="text-center mb-8">
            <span class="badge badge-pink mb-4" style="display:inline-block;">🛍️ Our Shop</span>
            <h2 style="font-size:clamp(28px,4vw,44px);color:var(--navy);">Books & Goodies</h2>
        </div>
        <div class="grid md:grid-4 gap-6">
            @foreach($shopProducts as $p)
                <a href="{{ $p->buyUrl() !== '#' ? $p->buyUrl() : route('shop') }}" {{ $p->buyUrl() !== '#' ? 'target=_blank' : '' }} class="card" data-reveal>
                    <div style="aspect-ratio:1;border-radius:12px;overflow:hidden;background:#fff;margin-bottom:12px;display:flex;align-items:center;justify-content:center;">
                        @if($p->image_url)<img src="{{ $p->image_url }}" alt="{{ $p->name }}" style="width:100%;height:100%;object-fit:cover;">
                        @else<span style="font-size:42px;">{{ $p->type=='BOOK'?'📚':'🎁' }}</span>@endif
                    </div>
                    <h3 style="font-size:15px;color:var(--navy);margin-bottom:4px;">{{ $p->name }}</h3>
                    <p style="font-weight:700;color:var(--pink);">{{ $p->currency }} {{ number_format($p->price,2) }}</p>
                </a>
            @endforeach
        </div>
        <div class="text-center mt-6">
            <a href="{{ route('shop') }}" class="btn-purple">Visit the Shop →</a>
        </div>
    </div>
</section>
@endif
